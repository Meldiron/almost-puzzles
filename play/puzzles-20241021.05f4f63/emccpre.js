...
var Module = {
	preRun: function () {
		...
		// </script>
		var envscript, k, v;
		for (envscript of document.querySelectorAll('script#environment, script.environment'))
			for ([k, v] of Object.entries(JSON.parse(envscript.textContent))) ENV[k] = v;
	},
...
function relative_mouse_coords(event, element) {
	...
	return { x: event.pageX - ecoords.x, y: event.pageY - ecoords.y };
}
...
function canvas_set_font(ctx, size, monospaced) {
	...
	ctx.font =
		`${s.fontStyle} ${s.fontWeight} ${size}px ` + (monospaced ? 'monospace' : s.fontFamily);
}
...
function initPuzzle() {
	...
	if (specific_button)
		specific_button.onclick = function (event) {
	...
	if (random_button)
		random_button.onclick = function (event) {
	...
	if (solve_button)
		solve_button.onclick = function (event) {
			if (dlg_dimmer === null) command(9);
		};
	if (prefs_button)
		prefs_button.onclick = function (event) {
	...
	function menukey(event) {
		...
		if (['1', '2', '3', '4', '5', '6', '7', '8', '9', '0'].includes(key)) {
			var shortcutitem = thismenu.querySelectorAll(":scope > li:not([role='separator']")[
				(Number(key) + 9) % 10
			];
			if (shortcutitem) {
		...
		if (ishorizontal(thismenu)) {
			// Top-level menu bar.
			if (key == 'ArrowLeft') targetitem = prevmenuitem(thisitem) || lastmenuitem(thismenu);
			else if (key == 'ArrowRight') targetitem = nextmenuitem(thisitem) || firstmenuitem(thismenu);
			else if (key == 'ArrowUp') targetitem = lastmenuitem(thisitem.querySelector('ul'));
			...
			if (parentitem) {
				if (ishorizontal(parentitem.closest('ul'))) parentitem_up = parentitem;
				else parentitem_sideways = parentitem;
			}
			if (key == 'ArrowUp')
				targetitem = prevmenuitem(thisitem) || parentitem_up || lastmenuitem(thismenu);
			else if (key == 'ArrowDown')
				targetitem = nextmenuitem(thisitem) || parentitem_up || firstmenuitem(thismenu);
			else if (key == 'ArrowRight')
				targetitem = thisitem.querySelector('li') || (parentitem_up && nextmenuitem(parentitem_up));
			else if (key == 'Enter') targetitem = thisitem.querySelector('li');
			else if (key == 'ArrowLeft')
				targetitem = parentitem_sideways || (parentitem_up && prevmenuitem(parentitem_up));
			else if (key == 'Backspace') targetitem = parentitem;
		}
		if (targetitem) targetitem.firstElementChild.focus();
		else if (key == 'Enter') target.click();
		else if (key == 'Escape' || key == 'SoftRight' || key == 'F10' || key == 'Backspace')
	...
	document.addEventListener('keydown', function (event) {
		...
		if (
			(event.key == 'SoftRight' || event.key == 'F10') &&
	...
	document.documentElement.addEventListener(
		...
		function (event) {
			...
			for (elem of Array.from(document.getElementsByClassName('focus-within')))
				if (!elem.contains(event.target)) elem.classList.remove('focus-within');
		},
		true
	);
