...
mergeInto(LibraryManager.library, {
	...
	js_set_colour: function (colour_number, colour_string) {
		...
		if (colour_number == 0)
			document.documentElement.style.setProperty('--puzzle-background', colours[colour_number]);
	},
	...
	js_canvas_end_draw: function () {
		if (update_xmin !== undefined) {
			var onscreen_ctx = onscreen_canvas.getContext('2d', { alpha: false });
			onscreen_ctx.drawImage(
				offscreen_canvas,
				update_xmin,
				update_ymin,
	 ...
	 */
	js_canvas_draw_text: function (x, y, halign, colour, fontsize, monospaced, text) {
	...
	js_load_prefs: function (me) {
		...
		for (var prefsscript of document.querySelectorAll('script.preferences'))
